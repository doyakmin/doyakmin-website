      ...
      <dl className="divide-y divide-gray-50">
        {data.map(({ label, value }) => (
          <div key={label} className="flex">
            <dt className="w-24 shrink-0 bg-gray-50/80 px-4 py-2.5 text-xs font-medium uppercase tracking-wider text-gray-500 sm:w-28">
              {label}
            </dt>
            <dd className="px-4 py-2.5 text-sm text-gray-600">{value}</dd>
          </div>
        ))}
      </dl>
    </div>
...
export default function PrivacyPage() {
  return (
    <main className="min-h-screen bg-gray-50/40 font-sans text-gray-900">
      {/* Hero */}
      <section className="border-b border-gray-100 bg-white">
        <div className="mx-auto max-w-3xl px-4 pb-10 pt-14 sm:px-6 md:pt-16 lg:px-8">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 md:py-12 lg:px-8">
        {/* Table of Contents */}
        <nav className="mb-10 rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 sm:grid-cols-3">
            {sections.map(({ id, label, icon: Icon }) => (
              <a
                key={id}
                href={`#${id}`}
                className="group flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-gray-600 transition-colors hover:bg-emerald-50 hover:text-emerald-700"
              >
                <Icon className="h-3.5 w-3.5 shrink-0 text-gray-400 transition-colors group-hover:text-emerald-600" />
        ...
        {/* Intro */}
        <div className="rounded-xl border border-gray-100 bg-white p-5 text-sm leading-relaxed text-gray-600 shadow-sm md:p-6 md:text-base">
        ...
        <div className="mt-8 space-y-6">
          {/* 제1조 */}
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-3 rounded-lg bg-gray-50 p-4 text-sm leading-relaxed text-gray-600 md:text-base">
              <p>
              ...
              <p className="mt-1.5">회사는 카드번호, 계좌번호 등 결제 수단 정보를 직접 수집·저장하지 않습니다.</p>
              <p className="mt-1.5">구독 상태(구독 여부, 만료일)만 서비스 제공 목적으로 저장됩니다.</p>
            </div>
          </article>
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-4 space-y-2.5">
              {[
                ...
                <div key={i} className="flex items-start gap-3 rounded-lg px-3 py-2 transition-colors hover:bg-gray-50">
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-4 space-y-2 text-sm leading-relaxed text-gray-600 md:text-base">
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <ul className="mt-3 space-y-2">
              {[
                ...
                <li key={item} className="flex items-start gap-2.5 text-sm text-gray-600 md:text-base">
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <ul className="mt-3 space-y-2">
              {[
                ...
                <li key={label} className="flex items-start gap-2.5 text-sm text-gray-600 md:text-base">
                  <ChevronRight className="mt-0.5 h-4 w-4 shrink-0 text-gray-400" />
                  <span>
                    <strong className="font-semibold text-gray-700">{label}</strong>: {desc}
                  </span>
                </li>
              ))}
            </ul>
            <div className="mt-5 rounded-lg border border-emerald-100 bg-emerald-50/60 p-4">
              <p className="text-sm text-gray-700 md:text-base">
                회원 탈퇴를 원하실 경우 앱 내{' '}
                <code className="rounded bg-white px-1.5 py-0.5 text-xs font-medium text-gray-700 shadow-sm">
                  설정 &gt; 계정 탈퇴
                </code>{' '}
                또는 아래 링크를 이용하세요.
              </p>
              <Link
                ...
                className="mt-2.5 inline-flex items-center gap-1.5 text-sm font-medium text-emerald-600 transition-colors hover:text-emerald-700"
              >
                계정 삭제 요청 페이지
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-4 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              {[
                ...
                <div key={i} className="flex items-center gap-2.5 rounded-lg border border-gray-100 px-3.5 py-2.5">
          ...
          <article className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
            ...
            <div className="mt-3 space-y-2">
              {[
                ...
                <div
                  key={name}
                  className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border border-gray-100 px-4 py-2.5"
                >
                  ...
                  <span className="text-gray-300">|</span>
                  <span className="text-sm text-gray-500">{tel}</span>
                </div>
              ))}
            </div>
          </article>
